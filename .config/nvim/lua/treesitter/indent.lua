-- Adapted from nvim-treesitter's indent module.
...
function M.get_indent(lnum)
   ...
   while node do
      ...
      local parent = should_process and node:parent() or nil
      ...
      if should_process and q['indent.align'][node:id()] and (srow ~= erow or is_in_err) and srow ~= lnum - 1 then
         ...
         if o_delim_node then
            ...
            elseif c_is_last_in_line and c_srow and o_srow ~= c_srow and c_srow < lnum - 1 then
               indent = math.max(indent - indent_size, 0)
            else
